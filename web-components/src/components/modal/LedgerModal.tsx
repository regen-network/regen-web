...
export default function LedgerModal({ summary, txRes, link, ...props }: LedgerModalProps): JSX.Element {
  ...
  return (
    <Modal {...props}>
      ...
      <Grid container alignItems="center" justify="center" className={classes.summaryTitle}>
      ...
      {summary.map((item: Item, index: number) => (
        <Grid key={index} container className={classes.summaryItem}>
      ...
      {txRes.tx && txRes.txResponse && (
        <>
          ...
          <div className={open ? classes.blockchainData : clsx(classes.blockchainData, classes.collapsed)}>
