...
export const getSellOrdersRow = (_: TranslatorType) => [
  <Box key="id" sx={{ width: '64px', whiteSpace: 'normal' }}>
  ...
  <Box key="price" sx={{ width: '86px', whiteSpace: 'normal' }}>
    {_(SELL_ORDERS_MAPPING[SELL_ORDERS_HEADERS.PRICE].name)}
  </Box>,
  <Box key="currency-denom" sx={{ width: '86px', whiteSpace: 'normal' }}>
    {_(SELL_ORDERS_MAPPING[SELL_ORDERS_HEADERS.CURRENCY_DENOM].name)}
  </Box>,
  <Box key="amount" sx={{ width: '70px', whiteSpace: 'normal' }}>
    {_(SELL_ORDERS_MAPPING[SELL_ORDERS_HEADERS.AMOUNT].name)}
  </Box>,
  <Box key="credit-class" sx={{ width: '70px', whiteSpace: 'normal' }}>
    {_(SELL_ORDERS_MAPPING[SELL_ORDERS_HEADERS.CREDIT_CLASS].name)}
  </Box>,
  <Box key="batch-denom" sx={{ width: '74px', whiteSpace: 'normal' }}>
  ...
  >
    <Box key="purchase-options-name" sx={{ mr: 1 }}>
  ...
  <Box key="batch-start-date" sx={{ width: '84px', whiteSpace: 'normal' }}>
    {_(SELL_ORDERS_MAPPING[SELL_ORDERS_HEADERS.BATCH_START_DATE].name)}
  </Box>,
  <Box key="batch-end-date" sx={{ width: '72px', whiteSpace: 'normal' }}>
    {_(SELL_ORDERS_MAPPING[SELL_ORDERS_HEADERS.BATCH_END_DATE].name)}
  </Box>,
  <Box key="seller">
    {_(SELL_ORDERS_MAPPING[SELL_ORDERS_HEADERS.SELLER].name)}
  </Box>,
];
