...
const getTextColor = (theme: Theme, disabled: boolean, dark: boolean): string => {
  if (dark) {
...
const getBorderColor = (theme: Theme, disabled: boolean, dark: boolean): string => {
  if (dark) {
    return disabled ? theme.palette.grey[100] : theme.palette.secondary.main;
  } else {
    return disabled ? theme.palette.grey[400] : theme.palette.secondary.contrastText;
  }
};

const getBackgroundColor = (theme: Theme, disabled: boolean, dark: boolean): string => {
  if (dark) {
    return disabled ? theme.palette.grey[100] : theme.palette.secondary.main;
  } else {
...
export default function PrevNextButton({
  ...
  return (
    <div className={styles.root} onClick={disabled ? undefined : onClick} aria-label={direction}>
      <ArrowDownIcon direction={direction} color={getTextColor(theme, disabled, dark)} />
    </div>
  );
}
