...
export const CollaboratorActionsDropdown: React.FC<CollaboratorActionsDropdownProps> =
  ({
    ...
    // Members context logic
    if (context === 'members') {
      // Only admins see dropdown, and only for themselves or others
    ...
    // Default project context (existing logic)
    ...
    if (isExternalAdmin) {
      if (isCurrentUser) {
        ...
        // Only show remove for users who are NOT part of the organization
        items = [
          {
            label: _(msg`Remove`),
            onClick: onRemove,
      ...
      // If orgRole exists, do not show any actions for external admin on org members
    } else {
      // Existing logic for organization members/collaborators
