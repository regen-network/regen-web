...
COPY docker/data/denom.json /home/ledger/data/

# Copy ecocredit state file
COPY docker/data/ecocredit.json /home/ledger/data/

# Add bank denom state to genesis
RUN jq '.app_state.bank.denom_metadata |= . + input' /root/.regen/config/genesis.json /home/ledger/data/denom.json > genesis-tmp.json
